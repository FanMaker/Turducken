...
- `openPath(_:)`, for queueing a destination as a bare path with no hostname convention. Mirrors the Android SDK's call of the same name.

#### Fixed
- Push destinations carrying an ordinary `https://` link to the site's own host are now routed into the SDK. `handleUrl(_:)` previously accepted only a literal `fanmaker` hostname, so a link reached the SDK only if whoever composed it used that convention. It now accepts the site's own host and any of its `allowed_domains`, alongside the legacy shape. See FanMaker/app#1885.
- Links out of the webview to non-first-party hosts are handed to the system browser by the SDK itself, rather than left to each host app to detect. Only links the fan taps and links opening a new window are ejected; redirects, form posts and subframes are untouched, so SSO and payment flows that route through third-party hosts still complete in place.
